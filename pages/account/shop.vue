<template>
    ...
    <UCard>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-2">
          ...
          <textarea
            v-model="new_shop.description"
            class="h-[200px] rounded-md p-3 border dark:border-gray-600 outline-none"
            placeholder="We sell all kinds of things..."
            required
          ></textarea>
        </div>
        <div class="flex flex-col gap-2">
          <b>Shop category</b>
          <select
            v-model="new_shop.category"
            class="p-3 border dark:border-gray-600 rounded-md"
          >
      ...
      <template #footer>
        <UButton
          :disabled="
            !new_shop.name || !new_shop.description || !new_shop.category
          "
          @click="createNewShop"
          color="green"
  ...
  >
    <UCard>
      ...
      <div class="flex justify-center items-center flex-col gap-3">
        <div
          class="w-fit rounded-full border-[6px] border-purple-500 p-1 relative"
        >
          <div
            :style="`background: url(${shop.image})`"
            class="size-[100px] rounded-full dark:border-gray-600 !bg-cover !bg-center !bg-no-repeat overflow-hidden"
          ></div>
          <span
            class="absolute size-[30px] -right-2 bottom-2 text-white rounded-full flex justify-center items-center bg-purple-500 p-3"
          >
        ...
        <div
          class="flex flex-wrap gap-4 justify-start items-center py-3 w-full"
        >
          <UButton
          ...
          <div class="flex justify-between items-center w-full">
            ...
            <span>=</span>
            <input
              class="border-b p-3 outline-none w-[85%]"
              placeholder="XXX"
              disabled
              :value="`${shop_boost_duration * 10} Coins`"
            />
          </div>
        </div>
      </div>
    ...
    <UCard>
      ...
      <div class="text-center flex flex-col gap-2">
        <span class="text-3xl font-bold"
          >Are you sure you want to cancel your shop boosting?</span
        >
        <span
          >Canceling your shop boosting means your shop will be removed from
          highlights and customers will stop seeing your shop visibly</span
        >
      </div>
      <template #footer>
        <div class="flex items-center justify-end gap-6">
      ...
      <div class="flex flex-row gap-3">
        ...
        <div class="flex flex-col">
          ...
          <span class="font-bold"
            >NGN {{ current_product.price.toLocaleString() }}</span
          >
        </div>
      </div>
      <template #footer>
      ...
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <UButton
            ...
            icon="i-heroicons-x-mark-20-solid"
            class="-my-1"
            @click="product_edit_modal = false"
          />
        </div>
      </template>
      <div class="flex flex-col gap-3">
        <div class="flex flex-row gap-3">
          ...
          <div class="flex flex-col">
            ...
            <div class="flex gap-2 mt-3">
              <img
                v-for="image in current_product.images"
                :src="image"
                class="w-[45px] rounded-md"
              />
            </div>
          </div>
        </div>
        <form @submit.prevent="updateProduct" class="flex flex-col gap-3">
          ...
          <div class="flex flex-col">
            ...
            <textarea
              class="border dark:border-gray-600 p-3"
  ...
  <div v-if="user?.account_type == 'seller'">
    ...
    <form
      ...
      class="border dark:border-gray-600 rounded-lg flex flex-col p-5 gap-3"
    >
      <div class="flex justify-center items-center mt-6">
        <div
          @click="image_cropper = true"
          :style="`background: url(${shop.image})`"
          class="size-[100px] border rounded-full dark:border-gray-600 !bg-cover !bg-center !bg-no-repeat relative overflow-hidden group"
        >
          <div
            class="h-full w-full absolute top-0 bg-black bg-opacity-30 group-hover:flex hidden justify-center items-center"
          >
      ...
      <div class="flex flex-col gap-3 mt-4">
        ...
        <UInput placeholder="Enter shop name" v-model="shop.name" disabled />
        <small class="text-blue-500"
          ><i class="bi bi-exclamation-circle"></i> Please contact support if
          you would want to change your shop name</small
        >
      </div>
        ...
        <div class="h-[400px] overflow-y-auto">
          <div class="flex flex-col gap-4 p-3">
            <div 
              ...
              :key="template.value"
              @click="shop.template_code = template.value"
              class="relative  border-opacity-25 rounded-lg p-4 cursor-pointer transition-transform"
              :class="{
                'border-green-500': shop.template_code === template.value,
                'bg-pink-50 dark:bg-pink-900/20 border-pink-200': index === 0,
                'bg-blue-50 dark:bg-blue-900/20 border-blue-200': index === 1, 
                'bg-purple-50 dark:bg-purple-900/20 border-purple-200': index === 2,
                'bg-orange-50 dark:bg-orange-900/20 border-orange-200': index === 3,
                'bg-teal-50 dark:bg-teal-900/20 border-teal-200': index === 4
              }"
            >
              <div class="flex items-center gap-3">
                <div class="w-16 h-16 rounded flex items-center justify-center"
                  :class="{
                    'bg-pink-100 dark:bg-pink-800': index === 0,
                    'bg-blue-100 dark:bg-blue-800': index === 1,
                    'bg-purple-100 dark:bg-purple-800': index === 2, 
                    'bg-orange-100 dark:bg-orange-800': index === 3,
                    'bg-teal-100 dark:bg-teal-800': index === 4
                  }"
                >
                  <i class="bi bi-layout-text-window-reverse text-2xl"></i>
                </div>
                <div>
                  <h3 class="font-medium">{{ template.name }}</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-300">Template {{ template.value }}</p>
                </div>
              </div>
              <div v-if="shop.template_code === template.value" class="absolute top-2 right-2">
    ...
    <div class="flex p-3 gap-3 border dark:border-gray-600 rounded-lg my-4">
      <UButton
        @click="
          [(currentTab = 'listings'), (useRoute().query.tab = 'listings')]
        "
        label="Listings"
        color="green"
    ...
    <!-- LISTINGS -->
    <div
      v-if="currentTab == 'listings'"
      class="border dark:border-gray-600 rounded-lg flex flex-col p-5"
    >
      <UTable
      ...
      <div
        class="flex gap-6 mt-1 w-full justify-center items-center"
    ...
    <!-- SPECIAL ACTIONS -->
    <!-- PROUDUCTS -->
    <div class="divider-tab">Special Actions</div>
      ...
      <div class="mt-4 flex flex-col gap-4">
        <div class="flex flex-row justify-between items-center">
          ...
          <UToggle
            @click.prevent="
              shop.is_boosted
                ? (cancel_boost_modal = true)
                : (boost_shop_modal = true)
            "
            color="purple"
        ...
        <div class="flex flex-row justify-between items-center">
          <div class="flex flex-col">
            ...
            <span
              >Expand reach with front row billboard access, pay per ad
              click</span
            >
          </div>
          <UToggle
  ...
  >
    ...
    <div class="flex flex-col justify-center items-center text-center">
      <div
        class="flex flex-col justify-center items-center size-[100px] bg-gray-900 bg-opacity-80 rounded-full text-gray-500"
      >
...
const prod_delivery_charge = ["yes", "no"];
...
const getShopProducts = async (shop_name = user.value.shop.name) => {
  ...
  try {
    const res = await useNuxtApp().$apiFetch(
      `/products/${shop_name}/shop?page=${current_page.value}`
    );
